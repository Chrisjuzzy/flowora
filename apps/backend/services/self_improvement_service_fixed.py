...
class SelfImprovementService:
    ...
    def load_agent_memory(db: Session, agent_id: int, current_prompt: str) -> str:
        """
        Load previous memory related to agent and current prompt
...
"
                memory_context += f"   Decision: {memory.decision[:100] if len(memory.decision) > 100 else memory.decision}...
"
                memory_context += f"   Outcome: {memory.outcome[:100] if len(memory.outcome) > 100 else memory.outcome}...
"
    ...
    def write_agent_memory(db: Session, agent_id: int, input_prompt: str, 
        ...
        try:
            ...
            memory = AgentMemory(
                agent_id=agent_id,
                query=input_prompt,
                decision=f"Generated response based on: {input_prompt[:100] if len(input_prompt) > 100 else input_prompt}",
            ...
            # Clear related cache
            import hashlib
            cache_key = f"agent_memory:{agent_id}:{hashlib.sha256(input_prompt.encode()).hexdigest()}"
...
{user_prompt}"
    ...
    def process_feedback(db: Session, agent_run_id: int, rating: int, 
        ...
        try:
            ...
            import hashlib
            cache_key = f"agent_memory:{agent_run.agent_id}:{hashlib.sha256(agent_run.input_prompt.encode()).hexdigest()}"
            cache.delete(cache_key)
